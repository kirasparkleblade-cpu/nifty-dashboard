(function(){
  ...
  async function fetchJSON(symbol, interval, range){
    // Use our own Vercel serverless API route (same origin) instead of
    // hitting Yahoo Finance directly through third-party CORS proxies.
    // Public proxies (allorigins/corsproxy/codetabs) get rate-limited or
    // blocked, which was causing the widget to hang on "Loading chart...".
    const apiUrl = `/api/nifty-candle?symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}&range=${encodeURIComponent(range)}`;
    const res = await fetch(apiUrl, { cache: 'no-store' });
    if (!res.ok) {
      let detail = '';
      try { detail = (await res.text()).slice(0, 200); } catch(_e){}
    ...
    if (json && json.chart && json.chart.error) throw new Error(json.chart.error.description || 'Yahoo error');
    throw new Error('Unexpected response shape from /api/nifty-candle');
  }
